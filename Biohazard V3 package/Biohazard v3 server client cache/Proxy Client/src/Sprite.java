...
public final class Sprite extends DrawingArea {
 ...
 public Sprite(String img) {
  ...
  private void drawAlphaSprite(int i, int j, int ai[], int ai1[], int l, int i1, int j1, int k1, int l1) {
	...
	/**
	 * Draws this sprite with the top {@code cropTop} rows skipped (used for orb fills).
	 */
	public void drawFromBottom(int x, int y, int cropTop) {
		if (myPixels == null || myWidth <= 0 || myHeight <= 0)
			return;
		if (cropTop < 0)
			cropTop = 0;
		if (cropTop >= myHeight)
		...
		int i1 = cropTop * myWidth;
		int j1 = myHeight - cropTop;
		int k1 = myWidth;
		int l1 = DrawingArea.width - k1;
		int i2 = 0;
