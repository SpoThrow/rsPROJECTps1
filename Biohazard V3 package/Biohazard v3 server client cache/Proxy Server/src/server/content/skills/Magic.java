...
public class Magic {
	...
	private static boolean hasRunes(Client c, int spellID) {
		...
		if (ens.getReq3() == 0) {
			if(c.getCombat().wearingStaff(ens.getReq1()) && c.getItems().playerHasItem(ens.getReq2(), ens.getReqAmt2()) && c.getItems().playerHasItem(ens.getReq3(), ens.getReqAmt3()) ||
				(c.getCombat().wearingStaff(ens.getReq2()) && c.getItems().playerHasItem(ens.getReq1(), ens.getReqAmt1()) && c.getItems().playerHasItem(ens.getReq3(), ens.getReqAmt3()) ||
				(c.getCombat().wearingStaff(ens.getReq3()) && c.getItems().playerHasItem(ens.getReq1(), ens.getReqAmt1()) && c.getItems().playerHasItem(ens.getReq2(), ens.getReqAmt2())))) {
				return true;
			} else if (c.getItems().playerHasItem(ens.getReq1(), ens.getReqAmt1()) && c.getItems().playerHasItem(ens.getReq2(), ens.getReqAmt2()) && c.getItems().playerHasItem(ens.getReq3(), ens.getReqAmt3())) {
					return true;
			} else {
				return false;
			}
			...
			if(c.getCombat().wearingStaff(ens.getReq1()) && c.getItems().playerHasItem(ens.getReq2(), ens.getReqAmt2()) ||
					(c.getCombat().wearingStaff(ens.getReq2()) && c.getItems().playerHasItem(ens.getReq1(), ens.getReqAmt1()))) {
				return true;
			} else if (c.getItems().playerHasItem(ens.getReq1(), ens.getReqAmt1()) && c.getItems().playerHasItem(ens.getReq2(), ens.getReqAmt2())) {
				return true;
			} else {
				return false;
			}
			...
			if (c.getItems().playerHasItem(enc.getUnenchanted(), 1)) {
				if (hasRunes(c, spellID)) {
					if (getEnchantmentLevel(spellID) == enc.getELevel()) {
						...
						c.getPA().addSkillXP(enc.getXp()*Config.MAGIC_EXP_RATE, Player.playerMagic);
			...
			for(int i = 0; i < boltData.length; i++) {
				if(spell == boltData[i][0]) {
					if(bolt == boltData[i][1]) {
						...
						c.getPA().addSkillXP(boltData[i][2] * amount *Config.MAGIC_EXP_RATE, 6);
