...
public class DesertTreasure {
	...
	public String nextDtKill(String lastKill) {
		...
		else if(Server.npcHandler.getNpcName(1975).equalsIgnoreCase(lastKill)) {
	...
	public int nextDtNpcId(String lastKill) {
		if(Server.npcHandler.getNpcName(1975).equalsIgnoreCase(lastKill)) {
			return 1914;
		} 
	...
	public void handleDtKills(int i) {
		if (NPCHandler.npcs[i].npcType == 1974) {
			getDtNpcInfo(1975);
		...
		for(int j = 0; j < dtNpcs.length; j++) {
			if(NPCHandler.npcs[i].npcType == dtNpcs[j]) {
				c.inDt = false;
				c.getPA().movePlayer(ARCH_X, ARCH_Y, 0);
				if(dtNpcs[j] != 1913) {
					c.getDH().sendDialogues(46, ARCH_ID);
				} else {
