...
public class ItemAssistant {
    ...
    public static String weapons[] ={"byssal tentacle","whip", "maul", "-om", "spear", "hammer", "sword",
        "rapier", "staff", "banner", "mace", "dagger", "scimitar", "spear",
        "flail", "maul", "axe", "bow", "knife", "tenderiser", "cannon",
        "chinchompa", "katana", "crossbow", "longsword",
        "claws", "greataxe"};
    
    public static int getWeaponCount(Client c) {
        int count = 0;
        for (int j = 0; j < c.playerItems.length; j++) {
            for(String a:weapons)
                if (ItemAssistant.getItemName(c.playerItems[j]).contains(a))
                    count++;
        }
    ...
    public static int getBodyCount(Client c) {
        int count = 0;
        for (int j = 0; j < c.playerItems.length; j++) {
            if(Item.playerBody(c.playerItems[j]))
            	count++;
        }
        if(c.playerEquipment[c.playerChest] > 0)
            count++;
        return count;
    }

    public static int getLegCount(Client c) {
        int count = 0;
        for (int j = 0; j < c.playerItems.length; j++)
            if(Item.playerLegs(c.playerItems[j]))
            	count++;
        if(c.playerEquipment[c.playerLegs] > 0)
            count++;
        return count;
    }
	
	public void replaceItem(int itemToReplace, int replaceWith) {
		if(c.getItems().playerHasItem(itemToReplace)) {
			c.getItems().deleteItem(itemToReplace, 1);
			c.getItems().addItem(replaceWith, 1);
		}
	}
	...
	public int getKeepNet(Client c) {
	...
	public int getLostNet(Client c) {
		return getKeepNet(c) - (getEquipmentNet(c) + getInventoryNet(c));
	}

	public int getTotalNet(Client c) {
		return getBankNet(c) + getEquipmentNet(c) + getInventoryNet(c);
	}
	
	public int getTotalNetPlayer(Client c) {
		return getEquipmentNet(c) + getInventoryNet(c);
	}
	...
	public int getInventoryNet(Client c) {
		...
		for (int i = 0; i < c.playerItems.length; i++) {
			toReturn += c.getShops().getItemShopValue(c.playerItems[i])
					* c.playerItemsN[i];
			if(c.playerItems[i] == 996)
				toReturn += 1 * c.playerItemsN[i];
		}
		return toReturn;
	}

	public int getBankNet(Client c) {
		int toReturn = 0;
		for (int i = 0; i < c.bankItems.length; i++) {
			toReturn += c.getShops().getItemShopValue(c.bankItems[i])
					* c.bankItemsN[i];
			if(c.bankItems[i] == 996)
				toReturn += 1 * c.bankItemsN[i];
		}
		return toReturn;
	}
	...
	*Project Innovation
	**/
	public int shardOne = 11710;
	public int shardTwo = 11712;
	public int shardThree = 11714;
	public boolean hasAllShards() {
	if(hasShardOne() && hasShardTwo() && hasShardThree()){
		return true;} return false; }
	
	
	public boolean hasShardOne() {
	...
	**/
	...
	{4724,4908},{4726,4914},{4728,4920},{4730,4926},{4745,4956},{4747,4962},{4749,4968},{4751,4794},
	...
	**/
	 public void addItemToBank(int itemId, int amount) {
		...
		int bankId = itemId + 1;
		int slot = c.getBank().findExistingSlot(bankId);
		if (slot != -1) {
			long total = (long) c.bankItemsN[slot] + amount;
			if (total > Config.MAXITEM_AMOUNT) {
	...
	public boolean addItem(int item, int amount) {
		//synchronized(c) {
			...
			if ((((freeSlots() >= 1) || playerHasItem(item, 1)) && Item.itemStackable[item]) || ((freeSlots() > 0) && !Item.itemStackable[item])) {
				...
				if(c.isBanking)
					c.sendMessage("@red@[@cr1@@red@]@bla@Bank 1 item and then do this command again.");
				return false;
			}
		//}
	}
		 ...
		 for (int j = 0; j < c.playerEquipment.length; j++) {
			if (c.playerEquipment[j] > -1) {
				for ( int k = 0; k < c.playerBonus.length; k++)
					c.playerBonus[k] += Server.itemHandler.ItemList[c.playerEquipment[j]].Bonuses[k];
				} 
			} 
		}
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		} else if (WeaponName.endsWith("whip") || WeaponName.endsWith("byssal tentacle")) {
		...
		} else if (WeaponName2.startsWith("dagger") || WeaponName2.contains("sword") || WeaponName2.contains("anchor")) {
	...
	**/
	...
	public void specialAmount(int weapon, double specAmount, int barId) {
		...
		c.getPA().sendFrame70(specAmount >= 70 ? 500 : 0, 0, (--barId));
		c.getPA().sendFrame70(specAmount >= 60 ? 500 : 0, 0, (--barId));
		c.getPA().sendFrame70(specAmount >= 50 ? 500 : 0, 0, (--barId));
		c.getPA().sendFrame70(specAmount >= 40 ? 500 : 0, 0, (--barId));
		c.getPA().sendFrame70(specAmount >= 30 ? 500 : 0, 0, (--barId));
		c.getPA().sendFrame70(specAmount >= 20 ? 500 : 0, 0, (--barId));
	...
	*/
	...
	public void rearrangeBank() {
		c.getBank().ensureInitialized();
	}
	
	public void resetBank(){
		synchronized(c) {
			c.getBank().ensureInitialized();
			int[] map = c.getBank().getDisplayMap();
			c.getOutStream().createFrameVarSizeWord(53);
			c.getOutStream().writeWord(5382);
			c.getOutStream().writeWord(BankTabs.DISPLAY_SIZE);
			for (int i=0; i<BankTabs.DISPLAY_SIZE; i++){
				int src = (map != null && i < map.length) ? map[i] : -1;
				int id = 0;
				int amt = 0;
				if (src >= 0 && src < Config.BANK_SIZE) {
					id = c.bankItems[src];
					amt = c.bankItemsN[src];
				}
				if (amt > 254){
					c.getOutStream().writeByte(255);
					c.getOutStream().writeDWord_v2(amt);
				} else {
	...
	private int newBankSlot(int bankId) {
		return c.getBank().insertNewItem(bankId, 0);
	}

	private void rollbackEmptyBankSlot(int slot) {
		if (slot < 0 || slot >= Config.BANK_SIZE) {
			return;
		}
		if (c.bankItems[slot] <= 0 || c.bankItemsN[slot] <= 0) {
			c.bankItems[slot] = 0;
			c.bankItemsN[slot] = 0;
		...
		if (!Item.itemIsNote[c.playerItems[fromSlot]-1]) {
			...
			if (Item.itemStackable[c.playerItems[fromSlot]-1] || c.playerItemsN[fromSlot] > 1) {
				...
				if (!alreadyInBank && freeBankSlots() > 0) {
						toBankSlot = newBankSlot(c.playerItems[fromSlot]);
						if (toBankSlot < 0) {
							c.sendMessage("Bank full!");
							return false;
						}
			...
			if (Item.itemStackable[c.playerItems[fromSlot]-1] || c.playerItemsN[fromSlot] > 1) {
				...
				if (!alreadyInBank && freeBankSlots() > 0) {
			       	toBankSlot = newBankSlot(c.playerItems[fromSlot]-1);
					if (toBankSlot < 0) {
						c.sendMessage("Bank full!");
						return false;
					}
	...
	public void fromBank(int itemID, int fromSlot, int amount) {
		if (!c.isBanking || amount <= 0) {
			return;
		}
		fromSlot = c.getBank().toAbsolute(fromSlot);
		if (fromSlot < 0 || fromSlot >= Config.BANK_SIZE) {
			return;
		}
		...
		boolean canNote = bankId >= 0 && bankId < Item.itemIsNote.length && Item.itemIsNote[bankId] && bankId != 6813;
		boolean note = c.takeAsNote && canNote;
		if (c.takeAsNote && !note) {
			c.sendMessage("This item can't be withdrawn as a note.");
		}
		int giveId = note ? bankId : (bankId - 1);
		boolean stackable = giveId >= 0 && giveId < Item.itemStackable.length && Item.itemStackable[giveId];
		if (note || stackable) {
			if (addItem(giveId, amount)) {
				c.bankItemsN[fromSlot] -= amount;
				finishBankWithdraw(fromSlot);
			}
			return;
		}
		while (amount > 0 && c.bankItemsN[fromSlot] > 0) {
			if (!addItem(bankId - 1, 1)) {
				break;
			}
	...
	**/
	...
	public boolean hasSpaceFor(int itemId, int amount) {
		...
		int slotsNeeded = 1; // Default: items stack in one slot if same ID
		
		// Check if item is stackable
		boolean isStackable = itemId < Item.itemStackable.length && Item.itemStackable[itemId];
		
		if (!isStackable) {
			// Non-stackable items need one slot per item
			slotsNeeded = amount;
		} else {
			// Stackable items need one slot, but check if player already has this item
			if (getItemAmount(itemId) > 0) {
				slotsNeeded = 0; // Already has this item, will stack
			} else {
				slotsNeeded = 1; // New item, needs one slot
			}
		}
