package server.game.minigames.treasuretrails;
...
public class TreasureTrails {
	...
	public int clue2[] = { 1052, 1052, 2599, 2601, 2603, 2605, 2607, 2609, 2611, 2613,
	...
	public int clue3[] = { 
			...
			2671, 2673, 2675, 2581, 1052, 1052, 1052, 7342, 7348, 7454, 7460, 7374, 7376, 7382,
	...
	public boolean itemOnObject(int o) {
		switch(o) {
			case 14360:
				if(c.getItems().playerHasItem(2694)) {
					c.getItems().deleteItem2(2694, 1);
					c.getItems().addItem(2779, 1);
					showitem(2779, "It turned into a casket!");
					return true;
				}
				return false;
		}
	...
	public boolean clueNpc(int npc) {
		if (npc == 1917 && c.getItems().playerHasItem(2678, 1)) {
			c.getItems().deleteItem2(2678, 1);
			c.getItems().addItem(lvl1(), 1);
			showitem(lvl1(), "The Bandit Shopkeeper gives you another clue!");
			c.level1 += 1;
			return true;
			...
			c.level1 += 1;
			return true;
		} else if (npc == 1265 && c.getItems().playerHasItem(2684, 1)) {
			c.getItems().deleteItem2(2684, 1);
			c.getItems().addItem(lvl1(), 1);
			showitem(lvl1(), "You hit the Rock Crab and see a new clue!");
			c.level1 += 1;
			return true;
			...
			c.level1 += 1;
			return true;
		} else if (npc == 1597 && c.getItems().playerHasItem(2683, 1)) {
			c.getItems().deleteItem2(2683, 1);
			c.getItems().addItem(lvl1(), 1);
			showitem(lvl1(), "Vannaka gives you another clue!");
			c.level1 += 1;
			return true;
		} else if (npc == 604 && c.getItems().playerHasItem(2685, 1)) {
			c.getItems().deleteItem2(2685, 1);
			c.getItems().addItem(lvl2(), 1);
			showitem(lvl2(), "Thurgo gives you another clue!");
			c.level2 += 1;
			return true;
		} else if ((npc == 9 || npc == 10) && c.getItems().playerHasItem(2687, 1)) {
			c.getItems().deleteItem2(2687, 1);
			c.getItems().addItem(2777, 1);
			showitem(2777, "You steal a casket of the guard!");
			return true;
		} else if (npc == 519 && c.getItems().playerHasItem(2691, 1)) {
			c.getItems().deleteItem2(2691, 1);
			c.getItems().addItem(lvl2(), 1);
			showitem(lvl2(), "Bob gives you another clue!");
			c.level2 += 1;
			return true;
		} else if (npc == 12 && c.getItems().playerHasItem(2692, 1)) {
		...
		/*} else if (npc == 5142 && c.getItems().playerHasItem(2694, 1)) {
			...
			c.getItems().addItem(2779, 1);
			showitem(2779, "Slash Bash gives you a casket.");
			return true;
		} else if (npc == 675 && c.getItems().playerHasItem(2696, 1)) {
			c.getItems().deleteItem2(2696, 1);
			c.getItems().addItem(lvl3(), 1);
			showitem(lvl3(), "The shipyard worker gives you another clue!");
			c.level3 += 1;
			return true;
		} else if (npc == 802 && c.getItems().playerHasItem(2697, 1)) {
			c.getItems().deleteItem2(2697, 1);
			c.getItems().addItem(lvl3(), 1);
			showitem(lvl3(), "Brother Jered gives you another clue!");
			c.level3 += 1;
			return true;
		} else if (npc == 941 && c.getItems().playerHasItem(2699, 1)) {
			c.getItems().deleteItem2(2699, 1);
			c.getItems().addItem(lvl3(), 1);
			showitem(lvl3(), "The Green DRagon gives you another clue!!");
			c.level3 += 1;
			return true;
		} else if (npc == 4906 && c.getItems().playerHasItem(2700, 1)) {
			c.getItems().deleteItem2(2700, 1);
			c.getItems().addItem(lvl3(), 1);
			showitem(lvl3(), "The Woodsman Tutor gives you another clue!");
			c.level3 += 1;
			return true;
		}
		return false;
	...
	public void getDigLoc(int x, int y) {
		...
		} if (c.getItems().playerHasItem(2689, 1)) {
			if (x == 2650 && y == 3229) {
				c.getItems().deleteItem2(2689, 1);
				c.getItems().addItem(2777, 1);
				showitem(2777, "You've found a casket!");
			}
		} if (c.getItems().playerHasItem(2693, 1)) {
			if (x == 3092 && y == 3226) {
	...
	public void clues(int clueScroll) {
		...
		case 2677:// lvl 1
			...
			break;
		case 2678:// lvl 1
			Clue("", "", "", "Speak to the Bandit Shopkeeper for",
					"your next clue.", "", "",
					"");// npc:1917
			break;
		case 2679:// lvl 1
		...
		case 2682:// lvl 1
			Clue("", "", "", "Search the crates in the Falador",
					"General Store.", "", "", ""); //11745 x2955 y3390
			break;
		case 2683:// lvl 1
			Clue("", "", "", "Speak to Vannaka in Edgeville Dungeon",
					"for your next clue.", "", "", ""); //npc:1597
			break;
		case 2684:// lvl 1
			Clue("", "", "", "Kill a rock crab for your next clue.", "", "", "",
					"");
			break;
			
		case 2685:// lvl 2
			Clue("", "", "This anagram reveals", "who to speak to next:", "",
					"UGRHOT", "", ""); //THURGO //smithing master
			break;
		case 2686:// lvl 2
			Clue("", "", "", "Search a crate on Catherby port.", "", "", "", "");
			break; //356 x2804 y3428
		case 2687:// lvl 2
			Clue("", "", "The Falador guards, they do not see,",
					"my thievery fingers steal all things,",
					"though I tend to aim at their pocketries.", "", "", ""); //pickpocket guard at falador
			break;
		case 2688:// lvl2
		...
		case 2689:// lvl2
			...
			break;
		case 2690:// lvl 2
			Clue("", "", "Dig in the bank of Canifis.",
					"In a black hole.",
					"", "", "", "");
			break;
		case 2691:// lvl 2
			Clue("", "", "", "Speak with Bob in Lumbridge.", "", "", "", "");
			break;
		case 2692:// lvl 2
			Clue("", "", "This anagram reveals", "who to speak to next:", "",
					"RABARINA", "", ""); //npc:12
		...
		case 2693:// lvl3
			...
			break;
		case 2694:// lvl 3
			Clue("", "", "Put this clue in a Cauldron",
					"on Pest Control Island.", "",
					"", "", ""); //object 14360
			break;
		case 2695:// lvl 3
			Clue("", "", "-It's so hot here...",
					"I need water!-",
					"This monster is so huge,",
					"I need to attack it", "to defence myself!", "");
			break;
		case 2696:// lvl 3
			Clue("", "", "This anagram reveals", "who to speak to next:", "",
					"IPSHDRAY ORKEWR", "", ""); //shipyard worker npc:675
			break;
		case 2697:// lvl 3
			Clue("", "", "", "Speak with Brother Jered for",
					"your next clue.", "", "", "");
			break;
		case 2698:// lvl3
