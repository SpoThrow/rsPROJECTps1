...
public class TradeAndDuel{
	...
	**/
	...
	public void openTrade() {
		...
		c.getPA().sendFrame126("Trading with: " + Misc.capitalize(c.playerName)+" " ,3417);
	...
	public boolean fromTrade(int itemID, int fromSlot, int amount) {
		...
		try {
			...
			if(!playerHasOfferedItem(itemID))
				return false;
					...
					for (GameItem item : offeredItems) {
						if(item.id == itemID) {	
							if(!item.stackable) {	
								...
								o.getPA().sendFrame126("Trading with: " + Misc.capitalize(c.playerName)+"", 3417);		
							} else {
	...
	**/
	...
	public void openDuel() {
		...
		c.getPA().sendFrame126("Dueling with: " + Misc.capitalize(o.playerName) + " (level-" + o.combatLevel + ")", 6671);
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		if (c.playerItems[fromSlot] - 1 != itemID && c.playerItems[fromSlot] != itemID) {
