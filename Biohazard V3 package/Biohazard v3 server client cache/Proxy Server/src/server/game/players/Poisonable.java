...
public class Poisonable {
    ...
    private static boolean isPoisionable(int wep) {
    ...
    public static boolean useItemonItem(Client c, int id, int id2) {
        ...
        if(isPoisionable(id)) {
            wep = ItemAssistant.getItemName(id); 
        ...
        if(isPoisionable(id2)) {
            wep = ItemAssistant.getItemName(id2);
        ...
        if(wep.toLowerCase().contains("(p++)") || wep.toLowerCase().contains("(p+)") || wep.toLowerCase().contains("(p)")) {
