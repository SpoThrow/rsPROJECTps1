...
public final class ObjectDef {
	...
	public static ObjectDef getObjectDef377(int i) {
		for (int j = 0; j < 20; j++)
			if (cache[j].type == i)
				return cache[j];
		byte[] buffer377 = archive377.get(i);
		if (i > buffer377.length)
			return getObjectDef(i);
		cacheIndex = (cacheIndex + 1) % 20;
		ObjectDef class46 = cache[cacheIndex];
		class46.type = i;
		class46.setDefaults();
	...
	public static void loadConfig() {
		...
		archive377 = new MemoryArchive(new ByteStream(getBuffer("loc377.dat")),
				new ByteStream(getBuffer("loc377.idx")));
...
11470,11471,136,11528,11529,11530,11531,1854,1000,9265,9264,1591,11708,11709,11851};
	...
	private void readValues(ByteStreamExt stream) {
	 ...
	 private void readValues377(ByteStreamExt stream)
	    {
	        int flag = -1;
		do {
			int type = stream.readUnsignedByte();
	...
	private static MemoryArchive archive377;
